...
export default function CheckoutForm({ herb }: CheckoutFormProps) {
  // Hardcoded to strictly use whatsapp
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Simulate quick secure generation delay
      ...
      <div className="max-w-2xl mx-auto bg-white border border-stone-200/60 rounded-2xl p-8 md:p-12 text-center shadow-lg transition-all duration-500 animate-fadeIn">
        ...
        <div className="max-w-md mx-auto space-y-4 text-stone-600 text-sm leading-relaxed mb-8">
          ...
          <p className="bg-brand-alertBg border border-amber-200 text-brand-alertTxt rounded-lg p-4 font-medium text-left">
            We have opened WhatsApp to finalize your payment verification via invoice. If the window did not open automatically, please check your browser's pop-up blocker or click the manual link to complete checkout.
          </p>
        </div>
        <div className="flex flex-col sm:flex-row justify-center gap-4">
          <a
  ...
  return (
    <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
      ...
      <div className="lg:col-span-7 space-y-8 bg-white p-6 md:p-8 rounded-2xl border border-stone-200/60 shadow-sm">
        ...
        {/* Informational Payment Notice Instead of Selector */}
        <div className="border-t border-stone-200/60 pt-6">
          <div className="p-5 rounded-xl border border-amber-200 bg-brand-alertBg text-brand-alertTxt text-sm space-y-3">
            <div className="flex items-center space-x-2 text-brand-alertTxt">
            ...
            <p className="text-xs leading-relaxed opacity-95">
              To offer a smooth regional payment setup, orders are settled directly with an operator via WhatsApp invoice. We support Mobile Money (Momo/OM), direct bank transfers, and local cash pick-up options.
            </p>
            <p className="text-xs font-semibold">
              Upon submittal, your invoice details wrap up securely and forward to WhatsApp automatically to complete fulfillment.
            </p>
          </div>
        </div>
      </div>
        ...
        >
          {status === 'submitting' ? (
            <>
              ...
              <span>Processing Order Info...</span>
            </>
          ) : (
